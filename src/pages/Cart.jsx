            ...
            <div>
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  <p className="text-[9px] font-black text-amber-500/60 uppercase tracking-widest mb-0.5 truncate">
                    {item.category}
                  </p>
                  ...
                  {item.variantLabel && (
                    <p className="text-[10px] font-bold text-amber-400 mt-0.5 uppercase tracking-widest">
              ...
              {SHOW_PRICES ? (
                <p className="text-sm font-black text-amber-400 mt-1 tabular-nums">
                  {fmt(item.price)}
                  <span className="text-[9px] text-zinc-600 font-normal ml-1">/ unidad</span>
                </p>
              ) : (
      ...
      <div className="flex items-center gap-2 mb-2">
        ...
        {isFree ? (
          ...
          <p className="text-xs font-bold text-white">
            Agrega <span className="text-amber-400 font-black">{fmt(remaining)}</span> más para envío gratis
          </p>
        )}
      </div>
      {!isFree && (
  ...
  return (
    <div className="bg-[#16161f] border border-white/5 rounded-2xl overflow-hidden">
      ...
      <div className="px-5 py-4 space-y-2.5">
        ...
        <div className="flex justify-between items-center">
          ...
          {SHOW_PRICES ? (
            ...
            <span className="text-[10px] text-amber-400 font-black uppercase tracking-widest">A cotizar</span>
          )}
        </div>
        ...
        {SHOW_PRICES && (
          <>
            {cartDiscount > 0 && (
              <div className="flex justify-between items-center">
                <span className="text-xs text-zinc-500 uppercase tracking-wide font-bold">Descuentos</span>
                <span className="text-sm font-bold text-red-400 tabular-nums">−{fmt(cartDiscount)}</span>
              </div>
            )}
            <div className="flex justify-between items-center">
              <span className="text-xs text-zinc-500 uppercase tracking-wide font-bold">Envío estimado</span>
              {isFree ? (
            ...
            <div className="border-t border-white/5 pt-3 flex justify-between items-end">
              <div>
                <p className="text-[9px] text-zinc-600 uppercase tracking-widest font-bold mb-0.5">Total estimado</p>
                <p className="text-[9px] text-zinc-600">Precios sujetos a confirmación</p>
              </div>
              <p className="text-2xl font-black text-amber-400 tabular-nums">{fmt(finalTotal)}</p>
            </div>
          </>
        )}
      ...
      <div className="px-5 pb-5 space-y-3">
        ...
        <p className="text-[10px] text-zinc-700 text-center leading-relaxed pt-1">
          Un asesor de <span className="text-zinc-500 font-bold">EL GANZO herramientas</span> validará tu pedido y te enviará la cotización por WhatsApp.
        </p>
      </div>
    </div>
  );
...
function EmptyCart({ onContinue }) {
  return (
    <div className="min-h-[70vh] flex flex-col items-center justify-center px-6 text-center">
      ...
      <button
        onClick={onContinue}
        className="px-8 py-3 bg-amber-600 hover:bg-amber-500 text-white text-xs font-black uppercase tracking-widest rounded-full transition-all active:scale-95"
      >
        Ver Catálogo
      </button>
    </div>
      ...
      <div className="bg-[#111118] border-b border-white/5 px-4 pt-6 pb-5">
        <div className="max-w-2xl mx-auto">
          <div className="flex items-center gap-1.5 mb-3">
            ...
            <span className="text-[10px] font-black text-amber-500/70 uppercase tracking-[0.25em]">
          ...
          <p className="text-xs text-zinc-600 mt-2 font-bold uppercase tracking-widest">
          ...
          <div className="bg-[#16161f] border border-white/5 rounded-2xl p-4 space-y-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <button className="text-[10px] text-amber-500 font-bold hover:underline mt-0.5">Cambiar dirección →</button>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <div className="w-7 h-7 rounded-lg bg-amber-500/10 flex items-center justify-center">
